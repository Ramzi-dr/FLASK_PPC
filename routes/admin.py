...
def init_admin_routes(db):
    ...
    logger.info("🔧 Admin environment collection initialized")
    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            if not auth or not auth.username or not auth.password:
                return (
                    jsonify(msg="❌ Missing or invalid authentication"),
                    401,
            ...
            if not user_doc or not pw_doc:
                logger.critical("❌ Admin credentials missing in database")
                return jsonify(msg="❌ Admin credentials not found in database"), 500

            if auth.username != user_doc["value"]:
                logger.warning(f"❌ Invalid admin username attempt: {auth.username}")
                return jsonify(msg="❌ Invalid username or password"), 403

            if not bcrypt.checkpw(auth.password.encode(), pw_doc["value"].encode()):
                logger.warning(f"❌ Invalid password attempt for user: {auth.username}")
                return jsonify(msg="❌ Invalid username or password"), 403

            return fn(*args, **kwargs)
        except Exception as e:
            logger.critical(f"basic_auth_required failed: {e}")
            return jsonify(msg="❌ Internal server error"), 500
    ...
    def wrapper(*args, **kwargs):
        try:
            if request.remote_addr not in ("127.0.0.1", "::1"):
                logger.warning(f"❌ Remote access blocked from {request.remote_addr}")
                return (
                    jsonify(
                        msg="❌ You are using a remote machine and this is only allowed from localhost"
                    ),
                    403,
                )
            return fn(*args, **kwargs)
        except Exception as e:
            logger.critical(f"local_only check failed: {e}")
            return jsonify(msg="❌ Internal server error"), 500
...
def set_token_expiry():
    try:
        ...
        if access_seconds is None and refresh_seconds is None:
            return (
                jsonify(
                    msg="❌ Must provide at least one valid access_* or refresh_* time parameter (second/minute/hour/day)"
                ),
                400,
            )
        ...
        logger.info(
            f"✅ Token expiry updated. Access: {access_seconds}, Refresh: {refresh_seconds}"
        )
        return jsonify(
        ...
        logger.critical(f"POST /set_token_expiry failed: {e}")
        return jsonify(msg="❌ Internal server error"), 500
