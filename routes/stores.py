...
from flask import Blueprint, request, jsonify, Response  # type: ignore
...
def init_store_routes(db):
    ...
    def get_stores():
        try:
            ...
            for doc in db.stores.find({}):
                ...
                ordered["production"] = doc.get("production", True)
    ...
    def create_store():
        ...
        try:
            ...
            clientID = str(data.get("CLIENTID", "")).strip().upper()
            ...
            # Handle PRODUCTION
            production_raw = data.get("PRODUCTION", True)
            if isinstance(production_raw, str):
                production_raw = production_raw.strip().upper()
                if production_raw not in ("TRUE", "FALSE"):
                    return jsonify(msg="❌ 'PRODUCTION' must be true/false"), 400
                production = production_raw == "TRUE"
            elif isinstance(production_raw, bool):
                production = production_raw
            else:
                return jsonify(msg="❌ 'PRODUCTION' must be true/false"), 400

            open_time = str(data.get("OPEN_TIME", "")).strip()
            ...
            if not is_valid_24h_time(open_time):
                return jsonify(msg="❌ 'OPEN_TIME' invalid format"), 400
            if not is_valid_24h_time(close_time):
                return jsonify(msg="❌ 'CLOSE_TIME' invalid format"), 400
            ...
            msg = "✅ Store created."
            if missing_users:
                msg += f" Non-existing users ignored: {', '.join(missing_users)}."
    ...
    def update_store():
        ...
        try:
            ...
            if not current_name:
                return jsonify(msg="❌ 'NAME' or 'CURRENT_NAME' required"), 400

            existing_store = db.stores.find_one({"name": current_name})
            if not existing_store:
                return jsonify(msg=f"❌ Store '{current_name}' not found"), 404

            new_name = data.get("NEW_NAME")
            if new_name:
                new_name = str(new_name).strip().upper()
                if new_name != current_name and db.stores.find_one({"name": new_name}):
                    return jsonify(msg=f"❌ Store '{new_name}' already exists"), 409
            else:
                new_name = current_name

            update_fields, changes_made = {}, []

            # Map incoming UPPER keys to correct DB field names (preserve casing)
            FIELD_MAP = {
                "CLIENTID": "clientID",
                "ADDRESS": "address",
                "OPEN_TIME": "open_time",
                "CLOSE_TIME": "close_time",
                # "USERS" changes are handled via dedicated endpoints
            }
            ...
            if "PRODUCTION" in data:
                ...
                if isinstance(prod_raw, str):
                    prod_raw = prod_raw.strip().upper()
                    if prod_raw not in ("TRUE", "FALSE"):
                        return jsonify(msg="❌ 'PRODUCTION' must be true/false"), 400
                    prod_val = prod_raw == "TRUE"
                elif isinstance(prod_raw, bool):
                    prod_val = prod_raw
                else:
                    return jsonify(msg="❌ 'PRODUCTION' must be true/false"), 400

                if prod_val != existing_store.get("production", True):
                    update_fields["production"] = prod_val
                    changes_made.append("production")

            # Times (simple set; same behavior as your original PUT)
            if "OPEN_TIME" in data:
                update_fields["open_time"] = str(data["OPEN_TIME"]).strip()
                changes_made.append("open_time")
            if "CLOSE_TIME" in data:
                update_fields["close_time"] = str(data["CLOSE_TIME"]).strip()
                changes_made.append("close_time")

            # Other fields via FIELD_MAP or fallback to lower (for any extra)
            for key, value in data.items():
                if key in {
                    ...
                    continue
                target = FIELD_MAP.get(key, key.lower())
                new_val = value.strip().upper() if isinstance(value, str) else value
                if existing_store.get(target) != new_val:
                    update_fields[target] = new_val
            ...
            if not updated_store:
                logger.critical("PUT /stores: updated store not found after update")
                return jsonify(msg="❌ Internal server error"), 500

            # ---- Build safe JSON response (no helpers; inline fixes) ----
            updated_store.pop("_id", None)

            # Convert nested camera ObjectIds to strings; ensure camera.name exists
            converted_cameras = []
            ...
            ordered_store["open_time"] = updated_store.get("open_time", "00:00")
            ordered_store["close_time"] = updated_store.get("close_time", "23:59")
            ordered_store["production"] = updated_store.get("production", True)
            ordered_store["cameras"] = converted_cameras
            # -------------------------------------------------------------

            logger.info(f"Updated store '{current_name}' fields: {changes_made}")
            return (
                jsonify(msg=f"✅ Store '{current_name}' updated", store=ordered_store),
                200,
            )
    ...
    def delete_store():
        ...
        try:
            ...
            if not store_names:
                return jsonify(msg="❌ 'name' must be a non-empty string or list"), 400
    ...
    def remove_users_from_store():
        ...
        try:
            ...
            store_name = str(data.get("store_name", "")).strip().upper()
            ...
            if user_email:
                if not isinstance(user_email, str) or not user_email.strip():
                    return jsonify(msg="❌ 'user_email' must be non-empty string"), 400
                users_to_remove = [user_email.strip().upper()]
            elif user_emails:
                if not isinstance(user_emails, list) or not user_emails:
                    return jsonify(msg="❌ 'user_emails' must be a non-empty list"), 400
                users_to_remove = [
                    e.strip().upper()
                ...
                if not users_to_remove:
                    return jsonify(msg="❌ 'user_emails' contains no valid emails"), 400
            else:
                return (
            ...
            msg_parts = []
            if removed_users:
                msg_parts.append(f"✅ Removed: {', '.join(removed_users)}")
            if not_found_users:
                msg_parts.append(f"❌ Not found: {', '.join(not_found_users)}")
            if not_in_store:
                msg_parts.append(f"❌ Not in store: {', '.join(not_in_store)}")
    ...
    def add_users_to_store():
        ...
        try:
            ...
            if isinstance(user_emails, str):
                ...
                return jsonify(msg="❌ 'user_email' must be string or list"), 400
            ...
            msg_parts = []
            if added_users:
                msg_parts.append(f"✅ Added: {', '.join(added_users)}")
            if already_in_store:
                msg_parts.append(f"ℹ️ Already in store: {', '.join(already_in_store)}")
            if missing_users:
                msg_parts.append(f"❌ Not found: {', '.join(missing_users)}")
